		...
		for city in city_list:
			for i in range(len(month_list)):
				...
				for j in range(0,len(result)-11,11):
					...
					cursor.execute("INSERT INTO airquality (id, city, aqi, pm25, pm10, so2, co, no2, o3, date, createdAt, updatedAt, version ) VALUES(concat('g-',uuid()),'" + city +"'," + record_aqi +"," + record_pm25 +"," + record_pm10 +"," + record_so2 +"," + record_co +"," + record_no2+"," + record_o3 +",'" +record_day + "',now(),now(),0)")
					connection.commit()
